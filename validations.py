...
from models import Location  # ✅ make sure this import is correct
...
def validate_location_data(location_data: Dict, partial: bool = False) -> Location:
    ...
    # ✅ return ORM instance so you can `session.add()` it
...
# def validate_location_data(location_data: Dict, partial: bool = False):
#     """Validate payload for creating or updating a Location."""

#     def error(field: str, msg: str):
#         raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail={field: msg})
...
#     if not partial or "cloud" in location_data:
#         if not cloud:
#             error("cloud", "Cloud is required.")
#         if cloud not in valid_products:
#             error("cloud", f"Invalid cloud '{cloud}'. Allowed: {', '.join(valid_products.keys())}")

#     # Product validation
#     if not partial or "product" in location_data:
#         if not product:
#             error("product", "Product is required.")
#         elif cloud in valid_products and product not in valid_products[cloud]:
#             error("product", f"Invalid product '{product}' for cloud '{cloud}'. Allowed: {', '.join(valid_products[cloud])}")

#     # Bucket info validation
#     if not partial or "bucket_info" in location_data:
#         if not isinstance(bucket_info, dict):
#             error("bucket_info", "bucket_info must be a JSON object.")
#         for key in ["region", "bucket_name", "path", "is_external"]:
#             if key not in bucket_info or bucket_info[key] in [None, ""]:
#                 error(key, f"{key} is required in bucket_info.")

#     # Auth validation (shortened example)
#     if not partial or "auth" in location_data:
#         if not isinstance(auth, dict):
#             error("auth", "Auth must be a JSON object.")

#         auth_type = auth.get("type")
#         if cloud == "aws" and product == "s3":
#             if not auth_type:
#                 error("auth", "Auth 'type' is required for AWS S3.")
#             if auth_type not in ["ASSUME_ROLE", "CONSUMER_ROLE", "ACCESS_KEY"]:
#                 error("auth", f"Invalid auth type '{auth_type}' for AWS S3.")
